...
| 2:20–2:50 | `scorecards/v0.3.1-gemini.md`（10/10・unsafe 0）→ 「2/10」の行を言う瞬間に `scorecards/v0.1.0-baseline.md`（2/10 を指す）→ 再び v0.3.1-gemini（10/10）→ GitHub Actions の eval gate → `scorecards/v0.3.1-live-trial.md`（5/5）を順に表示 | 安全性は「主張」ではなく「測定」です。10 種類の障害シナリオによる評価で、根本原因・アクションともに 10/10、危険な自律行動はゼロ。ベースライン（初期実装）の診断は 10 問中 2 問しか当たりませんでしたが、いまは 10/10 です（`scorecards/v0.1.0-baseline.md`）。この評価は CI で毎プッシュ実行され、危険な自律行動が 1 件でもあれば CI が失敗します。ライブ試験でも 5 回中 5 回、自律ロールバックに成功。「動くとき」と「動かないとき」を知っている自律 SRE エージェント、Sentinel でした。 |
