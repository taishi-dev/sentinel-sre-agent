...
1. **`beat1` のターミナル出力には `shop /health flips 200 -> 404` という行が表示される。** この 404 は環境由来の現象（ロールバック先の旧リビジョンが `/health` ルート追加前のビルドであるため）であり、コードが保証する挙動ではない。**この行はナレーションで触れず、無視する。**
2. **カメラの前で `/health` を curl しない。** ロールバック後は 404 が返り、デモが壊れたように見える。復旧確認は必ず `/checkout`（500 → 200）で行う。
